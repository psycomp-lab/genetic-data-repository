...
to_db = {
    'host': 'localhost',    # Indirizzo del server del database
    'database': 'ML_genetic_data',      # Nome del database
    'user': 'postgres',     # Nome utente
    'password': 'maurizio',     # Password
    'port' : 5432           # Porta di connessione
}
...
def copy_to_ML_database(from_db_params, to_db_params):
    try:
        ...
        init_database = "DROP TABLE IF EXISTS measurement;\n"
        init_database += "DROP TABLE IF EXISTS measurement_type;"
        init_database += "DROP TABLE IF EXISTS sample_type;"
        
        init_database += "CREATE TABLE measurement_type(\n"
        init_database += "measurement_id INT GENERATED BY DEFAULT AS IDENTITY,\n"
        init_database += "name VARCHAR(64) NOT NULL,\n"
        init_database += "unit VARCHAR(15),\n"
        init_database += "PRIMARY KEY(measurement_id),\n"
        init_database += "CONSTRAINT uc_measurement_type UNIQUE (name, unit)"
        init_database += ");\n"
        
        init_database += "CREATE TABLE sample_type(\n"
        init_database += "sample_id INT GENERATED BY DEFAULT AS IDENTITY,\n"
        init_database += "analysis_id VARCHAR(255) UNIQUE NOT NULL,\n"
        init_database += "PRIMARY KEY(sample_id)\n"
        init_database += ");\n"

        init_database += "CREATE TABLE measurement(\n"
        init_database += "sample_id INT NOT NULL,\n"
        init_database += "measurement_id INT NOT NULL,\n"
        init_database += "value VARCHAR(15) NOT NULL,\n"
        init_database += "CONSTRAINT pk_measurement PRIMARY KEY(sample_id, measurement_id),\n"
        init_database += "CONSTRAINT fk_sampleid\n"
        init_database += "  FOREIGN KEY(sample_id)\n"
        init_database += "    REFERENCES sample_type(sample_id)\n"
        init_database += ",\n"
        init_database += "CONSTRAINT fk_measurementid\n"
        init_database += "  FOREIGN KEY(measurement_id) \n"
        init_database += "    REFERENCES measurement_type(measurement_id)\n"
        init_database += ");\n"

        get_gene_expressions = "SELECT * from gene_expression_file;"
        ...
        to_cursor.execute(init_database)
        to_connection.commit()
        
        genes = set()
        
        from_cursor.execute(get_gene_expressions)
